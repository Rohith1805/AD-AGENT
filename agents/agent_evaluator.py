...
class AgentEvaluator:
    ...
    def execute_code(self, code: str, algorithm_name: str) -> CodeQuality:
        ...
        # 4) Auto-install missing packages (only if not present)
        try:
            self._ensure_dependencies(cleaned_code)
        except Exception as e:
            err = f"[DependencyInstallError] {e}"
            print(err)
            return CodeQuality(
                code=cleaned_code, algorithm=algorithm_name, parameters={}, std_output="",
                error_message=err, auroc=-1, auprc=-1, error_points=[], review_count=0
            )

        # 5) Execute with the same interpreter (ensures venv is used)
        print(f"\n[INFO] Running script with interpreter: {sys.executable}\n")
